...
router.post('/generate-session', requireAuth, async (req, res, next) => {
  try {
    const { modality, split, muscleGroups = [], goal, cardioType, environment, homeEquipment = [] } = req.body;
    ...
    const context = {
      ...
      sessionRequest: { modality, split, muscleGroups, goal, cardioType, environment, homeEquipment },
